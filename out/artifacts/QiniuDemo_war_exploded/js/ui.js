...
FileProgress.prototype.setComplete = function(up, info) {
    ...
    var isVideo = function(url) {
        var res, suffix = "";
        var vidioSuffixes = ["flv", "f4v", "mp4", "m3u8", "webm","ogg"];
        ...
        for (var i = 0, l = vidioSuffixes.length; i < l; i++) {
            if (suffix === vidioSuffixes[i]) {
                return true;
            }
        }
        return false;
    };
    var isVideo = isVideo(url);
    if (isVideo){
        str = str + "<div class='hash'><strong>View:</strong><a href='http://localhost:8080/ckplayer/ckplayer.swf?f=" + url + "' target='_blank'>Preview</a></div>";
    }
